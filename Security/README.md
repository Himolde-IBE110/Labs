## Security:
A cryptographic hash function (https://en.wikipedia.org/wiki/Cryptographic_hash_function) maps an arbitrarily sized (message) entry to a string of fixed length bits, called a hash digest or digest. A cryptographic scatter function should be:
...
In this paper we will be using the MD5, SHA and SHA256 algorithms implemented in the Python Cryptography Toolkit (Package Crypto): collection of cryptographic modules that implement various functions and algorithms.
